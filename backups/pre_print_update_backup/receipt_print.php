...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .page-viewport { padding: 24px 15px 60px; display: flex; flex-direction: column; align-items: center; gap: 20px; overflow-x: auto; max-width: 100%; }
  .receipt-sheet {
      width: fit-content;
      min-width: 80mm;
      max-width: 100%;
...
<div class="controls-bar">
    <button class="btn-print" id="btnDirectPrint" onclick="sendDirectPrint()" style="background:#16a34a;">
        ⚡ Direct Print
    </button>
...
async function sendDirectPrint() {
    ...
    try {
        ...
        if (data.success) {
            showToast(data.message || 'Printed directly to thermal printer!');
        } else {
            showToast(data.error || 'Direct print failed', true);
        }
    } catch (err) {
