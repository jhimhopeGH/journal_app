<?php
// receipt_direct_print_ajax.php - AJAX handler for direct 1-click ESC/POS raw printing to EPSON TM-T82
...
// Helper to generate lines for a single entry
...
try {
    ...
    // If client requested the raw payload for WebSerial / WebUSB printing
    ...
    // Send payload directly to the server-attached printer
    $res = sendRawEscPosJob($allPayload, $printer);

    if ($res['success']) {
        logActivity('print', 'ej', "Printed $count receipt(s) to [{$res['printer']}]");
        echo json_encode([
            'success' => true,
            'message' => "Successfully printed $count receipt(s) directly to [{$res['printer']}]!",
            'printer' => $res['printer'],
            'count'   => $count
        ]);
    } else {
        $printErr = $res['error'] ?? 'Print job failed';
        logActivity('print', 'ej', "FAILED receipt print to [$printer]: $printErr");
        echo json_encode([
            'success' => false,
            'error'   => $printErr,
            'printer' => $printer
