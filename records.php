    ...
    <main class="main-content">
        <div class="top-nav">
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                ...
                <span id="agentStatusBadge" title="Local Print Agent Status" style="display:inline-flex;align-items:center;gap:4px;padding:0 10px;height:26px;border-radius:13px;font-size:11px;font-weight:700;background:#374151;color:#9ca3af;white-space:nowrap;cursor:default;">
                    ⬤ Agent: checking…
                </span>
                <?php endif; ?>
            </div>
...
/**
 * Probe local agent. Returns true if agent is alive on port 12111.
 */
async function isAgentAlive() {
    if (IS_SERVER_MACHINE) return false;
    try {
...
/**
 * Single Z-Read direct print — tries local agent first, falls back to server.
 */
async function quickZreadDirectPrint(id, btn) {
    ...
    try {
        const alive = await isAgentAlive();
        if (alive) {
            // Step 1: get raw ESC/POS payload from server
            const payloadRes = await fetch('zread_direct_print_ajax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: id, action: 'get_raw_payload' })
            });
            ...
            // Step 2: POST to local agent
            const agentRes = await fetch(LOCAL_AGENT_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ base64: payloadData.payload_base64, printer: printer })
            });
            const agentData = await agentRes.json();
            if (!agentData.success) throw new Error(agentData.error || 'Agent print failed');
            if (btn) { btn.innerHTML = '✓ Sent!'; }
            setTimeout(() => { if (btn) { btn.disabled = false; btn.innerHTML = orig; } }, 2200);
        } else {
            // Fallback: server-side print
            ...
            if (data.success) {
                ...
                throw new Error(data.error || 'Server print failed');
            }
        }
    } catch (err) {
...
/**
 * Batch Z-Read direct print — tries local agent first, falls back to server.
 */
async function batchDirectPrintSelected() {
    ...
    try {
        const alive = await isAgentAlive();
        if (alive) {
            // Step 1: get combined raw payload from server
            const payloadRes = await fetch('zread_direct_print_ajax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ids: checked, action: 'get_raw_payload' })
            });
            const payloadData = await payloadRes.json();
            if (!payloadData.success || !payloadData.payload_base64) {
                throw new Error(payloadData.error || 'Server did not return raw payload');
            ...
            const agentData = await agentRes.json();
            if (!agentData.success) throw new Error(agentData.error || 'Agent batch print failed');
            if (btn) { btn.innerHTML = '✓ ' + checked.length + ' Sent!'; }
            setTimeout(() => { if (btn) { btn.disabled = false; btn.innerHTML = orig; } }, 2500);
        } else {
            // Fallback: server-side batch print
            const resp = await fetch('zread_direct_print_ajax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (data.success) {
                ...
                throw new Error(data.error || 'Server batch print failed');
            }
        }
    } catch (err) {
        if (btn) { btn.disabled = false; btn.innerHTML = orig; }
...
(async function probeAgentOnLoad() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            badge.title = 'Local Print Agent RUNNING\nPrinter: ' + (info.default || '?');
            badge.innerHTML = '⬤ Agent: <span style="color:#6ee7b7">LIVE</span>';
        } else { throw new Error('not ok'); }
    } catch (_) {
        badge.style.background = '#450a0a';
        badge.style.color = '#fca5a5';
        badge.title = 'Local Print Agent OFFLINE\nRun scripts/pos_cashier_agent.bat on this PC';
