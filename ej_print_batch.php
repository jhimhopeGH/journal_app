...
function formatSingleEjReceipt($entry, $tenderNameMap, $W, $settings = []) {
    global $pdo;
    ...
    // Ensure SKU descriptions are populated from AS400
    ...
    // Load Store Master & Register Master
    $storeMaster = null;
    ...
    $topDivider   = getTopDivider($topDivType, $topDivCustom, $W);
    if ($topDivider !== '') {
        $lines[] = $topDivider;
    }
...
<div class="page-viewport">
    ...
    <?php foreach ($entries as $e): ?>
        <?php 
        $rendered = formatSingleEjReceipt($e, $tenderNameMap, $W, $settings); 
        $linesPerPage = (int)($settings['lines_per_page'] ?? ($paperSize === 'letter' ? 39 : 42));
        if ($linesPerPage <= 0) {
            $linesPerPage = $paperSize === 'letter' ? 39 : 42;
        }
        $entryChunks = array_chunk($rendered['lines'], $linesPerPage);
        if (empty($entryChunks)) {
            $entryChunks = [[]];
        }
        $totalEntryPages = count($entryChunks);
        ?>
        <?php foreach ($entryChunks as $pIdx => $chunkLines): ?>
        <div class="letter-sheet">
            ...
            <div class="receipt-container">
                ...
                <!-- Meta Bar: Only on first page of entry -->
                <?php if ($pIdx === 0): ?>
                ...
                <div class="receipt-body-wrapper" <?php echo ($pIdx < $totalEntryPages - 1) ? 'style="border-bottom: none;"' : ''; ?>>
